...
class Utils:
    ...
    all_colors = ['red', 'orange', 'yellow', 'green', 'blue', 'purple', 'black', 'fuchsia', 'violet', 'royalblue', 'navy',
                  'mediumspringgreen']
    ...
    def assign_markers_and_colors(no_clusters):  # for scatter plot
        ...
        for i in range(no_clusters):
            markers_and_colors.append(
                (i, Utils.all_markers[i % len(Utils.all_markers)], Utils.all_colors[i % len(Utils.all_colors)], Utils.all_colors_rgb[i % len(Utils.all_colors_rgb)]))
    ...
    @staticmethod
    def get_colors_array_with_clusters(samples, markers_and_colors, samples_nparray, clusters_nparray):
        colors_array = []
        for s in samples:
            index = np.where(samples_nparray == s)[0][0]
            cluster = clusters_nparray[index]
            color = markers_and_colors[cluster][3]
            colors_array.append(color)
        return colors_array

    @staticmethod
    def get_colors_array_with_clustersV2(indexes_array, markers_and_colors, samples_with_clusters):
        colors_array = []
